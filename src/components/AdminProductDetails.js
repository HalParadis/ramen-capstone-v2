...
const AdminProductDetails = ({ token, fetchRamen }) => {
  ...
  const updateRamen = async () => {
    const newRam = await updateRamenFromAPI({
      id: productId,
      token,
      name,
      price,
    ...
    <div className="product-details-page">
      {isEditMode ? (
        ...
        <div className="product-details">
          <img className="product-page-img" src={imgURL} />
          <div className="product-all-but-img">
            <h2 className="product-page-name">{name} </h2>
            <h3 className="product-page-name">{brand} </h3>
            <p className="product-page-name">{description} </p>
            <h3 className="product-page-name">{price} </h3>
          </div>
        </div>
      )}
      <div className="admin-edit-delete-buttons">
        <button type="button" onClick={() => setIsEditMode(!isEditMode)}>
          {isEditMode ? "Exit Edit Mode" : "Edit Ramen Info"}
        </button>
